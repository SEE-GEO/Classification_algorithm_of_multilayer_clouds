...
for k=1:lMLC
    ...
    a = find(isnan(notSeeding(k).Seeding)==0);
    notSeeding(k).seedAnz= length(a);    
end
...
%%
for k = 1:lMLC
if length(notSeeding(k).Zcloudbelow) == 0
      notSeeding(k).Zcloudbelow = 0;
      notSeeding(k).Zcloudabove = 0;
      notSeeding(k).Znoncloud = 0;
end
end
%%
...
for k=1:lMLC
    ...
    if isnan(notSeeding(k).Seeding) ==false
        ...
        lS = length(notSeeding(k).Zcloudbelow);
        Zcloudbelow(k,1:lS)=notSeeding(k).Zcloudbelow(1:lS);
        lS = length(notSeeding(k).Znoncloud);
        Znoncloud(k,1:lS)=notSeeding(k).Znoncloud(1:lS);
        lS = length(notSeeding(k).Zcloudabove);
...
title(strxcat('Not Seeding cases, r=',Rsize,'\mum'))
