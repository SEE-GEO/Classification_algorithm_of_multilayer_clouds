function [Raso,NoRasoNum] = Raso_2_test_loop(Rasohelp,idx_dayl,day,month,year,hour,minute,sek,altitude,temp,press,RHl,lat,lon,NoRasoNum,idx_dayk, iDate, lDate)
...
%Write in raso-struct:
     ...
     if isnan(Rasohelp.date(iDate,:)) == 0
        Raso(iDate).date=datestr(Rasohelp.date(iDate,:)); %Time of raso launch
        idx_begin=idx_dayl(iDate);
...
%Write time into raso-struct:
    ...
    %write variables into raso-struct und convert in SI:   
         ...
         if length(lat) < length(press)
             a = NaN(length(press),1);
             a(1:length(lat)) = lat;
             lat = a; clear a
             a = NaN(length(press),1);
             a(1:length(lon)) = lon;
