...
for k=1:lMLC
    ...
    Liquid(k).Nlayer=MLC_classification(k).liquid_super;
end
...
for k=1:lMLC
    ...
    a = find(isnan(SeedingS(k).Seeding)==0);
    SeedingS(k).seedAnz= length(a); 
    clear a
end
...
%%
for k = 1:lMLC
if length(SeedingS(k).Seeding) == 1
  if SeedingS(k).Seeding == 0
      SeedingS(k).Seeding = NaN;
  end
end
end
%%
for k = 1:lMLC
if length(SeedingS(k).Zcloudbelow) == 0
      SeedingS(k).Seeding = 0;
      SeedingS(k).Zcloudbelow = 0;
      SeedingS(k).Zcloudabove = 0;
      SeedingS(k).Znoncloud = 0;
end
end
%%
for k=1:lMLC
    if isnan(SeedingS(k).Seeding(1)) == 0
        lS=length(SeedingS(k).Seeding);
        Seed(k,1:lS)=SeedingS(k).Seeding(1:lS);
        lS = length(SeedingS(k).Zcloudbelow);
        Zcloudbelow(k,1:lS)=SeedingS(k).Zcloudbelow(1:lS);
        lS = length(SeedingS(k).Znoncloud);
        Znoncloud(k,1:lS)=SeedingS(k).Znoncloud(1:lS);
        lS = length(SeedingS(k).Zcloudabove);
...
txt={['1. No cloud above, no cloud in between, no cloud below: ']; ...             %Anz1 
    ['2. No cloud above, no cloud between, cloud below: ']; ...                    %Anz2
...
title(strxcat('Seeding cases, r=',Rsize,'\mum'))
